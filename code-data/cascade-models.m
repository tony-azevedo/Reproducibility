...
NumSteps =  [6 10 14];              % number shutoff steps
RhTCon = [200 300 400];             % rhodopsin decay time constant in ms
RhDecayFact = [0 0.25 0.5 0.75 1];  % extend of rhodopsin decay for each step
NumResponses = 10000;       % with 5000 resps simulation std ~1%, about 10% std in likelihood
...
Verbose = 1;
TransRate = [0.03 0.1];
TransCompression = 200;
TransRateCompressFlag = 1;
CovarFit = 0;
...
%                     SimulateCondition.TransCompression = 0;
%                     SimulateCondition.TransRateCompression = TransCompression(comp);
%                 else
%                     SimulateCondition.TransCompression = TransCompression(comp);
...
%                 end
    ...
    [MaxVal, MaxLoc] = max(Likelihood(:));
    MaxLoc = MaxLoc-1;
    Index1 = floor(MaxLoc / (length(TransCompression) * length(RhTCon)));
    Index2 = floor((MaxLoc - Index1 * (length(TransCompression) * length(RhTCon))) / length(TransCompression));
    Index3 = MaxLoc - Index1 * (length(TransCompression) * length(RhTCon)) - Index2 * length(TransCompression);
...
for step = 1:length(NumSteps)
    subplot(1, length(NumSteps), step);
    contour(RhTCon, TransCompression, log10(Likelihood(:, :, step)), sort(ContourLevels))
    temp = strcat(num2str(NumSteps(step)), ' steps');
    text(RhTCon(2), TransCompression(2), temp);
end
subplot(1, length(NumSteps), 1)
...
RhDecayFact = 1;
NumResponses = 1000;
TransCompression = [0 1e-5 3e-5 1e-4 3e-4 1e-3 3e-3 1e-2 3e-2 0.1];
TransRateCompressFlag = 1;
TranTCon = 50;
TransRate = 1;

fprintf(1, 'Remember to fix finite length effects in subroutine\n');
clear Likelihood;
